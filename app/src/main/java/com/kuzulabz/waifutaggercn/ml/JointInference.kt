...
class JointInference(
    ...
    /** 多模型融合器 */
    private val multiModelFuser = MultiModelFuser()
    ...
    private suspend fun runTwoStagePipeline(
        ...
        // Stage 5: 角色评分 + 排序
        onProgress(0.75f, s(R.string.workflow_progress_scoring))
        ...
        // 过滤标签
        onProgress(0.96f, s(R.string.workflow_progress_filtering))
    ...
    private suspend fun runDetectOnlyPipeline(
        ...
        val crop = det.cropDetectedRegion(bitmap, detections)
            ?: run {
                ...
                val tags = tagger.tag(bitmap, threshold, generalWeight, characterWeight)
                val filtered = tagFilter.process(tags)
                val limited = tagSelector.select(filtered, limit = 30)
                return JointResult(
        ...
        // 智能长度控制
        val limited = tagSelector.select(merged, limit = 30)
