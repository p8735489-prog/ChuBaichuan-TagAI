...
class WorkflowEngine(
    ...
    suspend fun execute(
        ...
        var characters: List<CharacterInstance> = emptyList()
        var totalTagsBeforeFilter = 0
        ...
        try {
                    ...
                    StepType.TAG -> {
                        if (segCrops.isNotEmpty()) {
                            ...
                            segCrops.forEachIndexed { i, crop ->
                                ...
                                totalTagsBeforeFilter += cropTags.size
                                // 角色隔离：每角色独立过滤
                                val filteredTags = tagFilter.processSingleCharacter(cropTags)
                                // 构建角色实例
                                val inst = if (i < segInstances.size) segInstances[i] else null
                                characterInstances.add(CharacterInstance(
                                    id = i,
                                    bbox = inst?.bbox ?: android.graphics.RectF(),
                                    mask = inst?.mask,
                                    ...
                                    area = characterScorer.calculateAreaRatio(
                                        inst?.mask, bitmap.width, bitmap.height
                                    ),
                                    className = crop.className,
                                    classId = crop.classId
                                ))
                                log.add("  Subject ${i + 1} (${crop.className}): ${cropTags.size} → ${filteredTags.size} tags")
                            }
                            // 角色评分 + 排序
                            characters = characterScorer.scoreAndRank(
                            ...
                            val bgTags = tagger.tag(bitmap, threshold, generalWeight, characterWeight)
                                .filterPromptNoiseTags()
                            // 融合 + 过滤
                            val limits = TagFilter.TagLimits(instanceCount = characters.size)
                            val merged = tagFilter.fuseAndFilter(
                                subjectTagsList = characters.map { it.filteredTags },
                            ...
                            // 长度控制
                            tags = lengthController.limitByScore(
                                merged, PromptLengthController.OutputMode.PRECISION
                            )
                            log.add("Tags: ${tags.size} (${segCrops.size + 1} regions scored & fused)")
                        } else {
                            onProgress(progress + 0.05f, s(R.string.workflow_progress_tagger))
                            val rawTags = tagger.tag(currentBitmap, threshold, generalWeight, characterWeight)
                                .filterPromptNoiseTags()
                            totalTagsBeforeFilter = rawTags.size
                            tags = tagFilter.process(rawTags)
                            tags = lengthController.limitByScore(
                                tags, PromptLengthController.OutputMode.NORMAL
                            )
            ...
            val debugInfo = if (characters.isNotEmpty()) {
                outputFormatter.formatDebug(
                    characters = characters,
                    totalTagsBeforeFilter = totalTagsBeforeFilter,
                    totalTagsAfterFilter = tags.size,
                    pipeline = "Workflow Engine"
                )
            } else ""
            return WorkflowResult(tags, currentBitmap, log, success = true, errorMessage = null,
                characters = characters, debugInfo = debugInfo)
        } catch (e: Exception) {
