    ...
    suspend fun execute(
        ...
        try {
            for ((index, step) in workflow.steps.withIndex()) {
                ...
                val stepName = step.type.displayNameZh
                onProgress(progress, "执行: $stepName…")

                when (step.type) {
                    StepType.INPUT -> {
                        log.add("输入图片: ${bitmap.width}x${bitmap.height}")
                    }
                    StepType.DETECT -> {
                        if (!detEngine.isReady) {
                            val err = detEngine.load()
                            if (err != null) {
                                log.add("检测模型加载失败: $err，跳过")
                                continue
                            }
                        }
                        detections = detEngine.detectObjects(currentBitmap)
                        val classSummary = detections.map { it.className }.distinct()
                        log.add("检测: 发现 ${detections.size} 个目标 (${classSummary.joinToString(", ")})")
                    }
                    StepType.SEGMENT -> {
                        val currentSeg = segEngine
                        if (currentSeg == null || !currentSeg.isReady) {
                            log.add("分割模型未加载，跳过分割步骤")
                            continue
                        }
                    ...
                    StepType.TAG -> {
                        if (segCrops.isNotEmpty()) {
                            ...
                            segCrops.forEachIndexed { i, crop ->
                                ...
                                log.add("  主体${i + 1}（${crop.className}）: ${cropTags.size} 个标签")
                            }
                            // 也对原图打标获取背景标签
                            ...
                            log.add("标签识别: ${tags.size} 个标签（${segCrops.size + 1} 区域融合）")
                        } else {
                            tags = tagger.tag(currentBitmap, threshold, generalWeight, characterWeight)
                                .filterPromptNoiseTags()
                            log.add("标签识别: ${tags.size} 个标签")
                        }
                    }
                    StepType.TRANSLATE -> {
                        log.add("标签翻译已标记")
                    ...
                    StepType.OUTPUT -> {
                        onProgress(0.98f, "生成提示词…")
                        log.add("输出: ${tags.size} 个标签")
                    }
                }
            }
            onProgress(1f, "完成")
    ...
    fun jsonToWorkflow(json: String): Workflow? {
        return try {
            ...
            Workflow(
                ...
                name = obj.optString("name", "自定义工作流"),
                steps = steps
            )
        } catch (e: Exception) {
