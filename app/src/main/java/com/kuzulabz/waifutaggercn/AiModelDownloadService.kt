...
class AiModelDownloadService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ...
            null -> {
                // 服务被系统杀死后重启（START_STICKY），尝试恢复下载
                if (pendingDownloadIntent != null && !isRunning) {
                    Log.i(TAG, "Service restarted by system, resuming download")
                    pendingDownloadIntent?.let { startDownload(it) }
    ...
    private fun acquireWakeLock() {
        ...
        wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "$TAG:download").apply {
            ...
            acquire(10 * 60 * 1000L) // 最多持有 10 分钟，防止泄漏
        }
    }
